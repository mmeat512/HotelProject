    ...
    <div class="modal fade" id="reserveModal" role="dialog">
       ...
       <script>
    	...
    	function totalgetList(){
    		$.getJSON(
    			"totalgetList/"+searchType+"/"+searchName,
    			function(data){
    				var str="";
    				console.log(data);
		...
		//모달창 띄우기
		$("#tobodyTot").on("click","a",function(event){
		...
		function reservemodal(reserveNum){
			$.getJSON(
				...
				function(data){
					...
					str += "<tr>";
					str += "<td>객실 번호</td>";
					str += "<td><input type='text' id='roomNum' name='roomNum' value='"+data.roomNum+"' readonly></td>";
					str += "</tr>";
					str += "<td>예약자</td>";
					...
					str += "<tr>";
					str += "<td>체크인</td>";
					str += "<td><input type='text' id='reserveCheckin' name='reserveCheckin' value='"+data.reserveCheckin+"'>";
					str += "</tr>";
					str += "<tr>";
					str += "<td>체크아웃</td>";
					str += "<td><input type='text' id='reserveCheckout' name='reserveCheckout' value='"+data.reserveCheckout+"'>";
					str += "</tr>";
					str += "<tr>";
					str += "<td>객실 타입</td>";
					...
					//객실타입과 침대타입에 따라 달라지는 select문 변경
					if(data.roomType == '스탠다드'){
						$("#op1").show();
						$("#op2").show();
						$("#op3").hide();
						$("#op4").show();
						
						$("#op00").attr("selected","selected");
							//스탠다드선택시 침대타입변경
							if(data.bedType == '더블'){
								$("#op1").attr("selected","selected");
							}else if(data.bedType == '트윈'){
						...
						if(data.roomType == '디럭스'){
							$("#op01").attr("selected","selected");
						}else if(data.roomType == '프리미어 디럭스'){
							$("#op02").attr("selected","selected");
						}else{
							$("#op03").attr("selected","selected");
						}
					} 
					//예약수정,취소 버튼 숨기기(취소된 예약 경우)
					if(data.reserveState == 'delete' || data.reserveState == 'past'){
						$("#aa").css("display","none");
						$("#bb").css("display","none");
					}else{
		...
		//객실타입 선택시 바뀌는 침대타입 변경
    	function changeBed(obj){
    		var getObj = obj[obj.selectedIndex].innerHTML;
    		
			if(getObj == '스탠다드' || $("#roomType").val() == '스탠다드'){
				$("#bedType").val("더블");
				$("#op1").show();
				$("#op2").show();
				$("#op3").hide();
				$("#op4").show();
			}else if(getObj == '디럭스' || getObj == '프리미어 디럭스' || getObj == '그랜드'){
				$("#bedType").val("더블/트윈");
				$("#op1").hide();
				$("#op2").hide();
				$("#op3").show();
		 ...
		 //모달창 수정(예약 수정)
    	function reUpdate(){
    		if($("#reserveCheckin").val() == ''){
    			alert("체크인시간을 입력하세요");
    		}else if($("#reserveCheckout").val() == ''){
    			$("#reserveCheckout").css("border-color","salmon");
    			alert("체크아웃시간을 입력하세요");
    		}else if($("#reserveAdult").val() == ''){
