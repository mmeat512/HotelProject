...
<jsp:include page="../include/header.jsp"></jsp:include>
    ...
    <div class="modal fade" id="reserveModal" role="dialog">
	  ...
	  <script>
    	...
    	function RoomgetList(){
    		$.getJSON(
    			"RoomgetList/"+searchType+"/"+searchName,
   				function(data){
    				console.log(data);
   					var str="";
    		...
    		//객실버튼클릭
	    	$("#tableTbody").on("click",".bbt",function(event){
				event.preventDefault(); //a태그 이동 막음
				console.log(event.target.getAttribute("href"));
				var reserveNum=event.target.getAttribute("href");
				location.href='adminSelect?reserveNum='+reserveNum;
			});
    		
    		//모달창 띄우기
    		$("#tableTbody").on("click","#td-12",function(event){
    			event.preventDefault(); //a태그 이동 막음
    			console.log(event.target.getAttribute("href"));
    			var reserveNum=event.target.getAttribute("href");
	...
	function reservemodal(reserveNum){
    	...
    	function timeStamp(milis){
    		...
    		var time = year+"/"+(month.toString().length == 1 ? '0'+month : month)+"/"+(day.toString().length == 1 ? '0'+day : day) ;
